...
const CameraCard = ({ name, alert, alertType }) => {
  return (
    <div
      className={`border rounded-xl p-4 transition-all duration-200 ${alert ? "border-red-500 bg-red-50" : "border-gray-200 hover:border-gray-300"}`}
    >
      <div className="flex justify-between items-start mb-3">
        ...
        <div
          className={`w-3 h-3 rounded-full ${alert ? "bg-red-500 animate-pulse" : "bg-green-500"}`}
        ></div>
      </div>

      {/* Camera Preview */}
      <div
        className={`h-32 rounded-lg mb-3 relative overflow-hidden ${alert ? "bg-gradient-to-br from-red-100 to-red-200" : "bg-gradient-to-br from-gray-100 to-gray-200"}`}
      >
      ...
      <div className="flex justify-between items-center">
        <span
          className={`text-xs font-medium px-2 py-1 rounded ${alert ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}
        >
          {alert ? "ALERT" : "ACTIVE"}
        </span>
...
const AlertCard = ({ level, title, location, time, color }) => {
  return (
    <div
      className={`border-l-4 p-4 rounded-r-lg ${color === "red" ? "border-red-500 bg-red-50" : "border-orange-500 bg-orange-50"}`}
    >
      <div className="flex justify-between items-start mb-2">
        <span
          className={`text-xs font-semibold px-2 py-1 rounded ${color === "red" ? "bg-red-200 text-red-700" : "bg-orange-200 text-orange-700"}`}
        >
          {level}
        </span>
              ...
              <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-6 gap-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    Complete incident tracking and management
                  </p>
                </div>
